...
defineFeature(feature, test => {
  // #Scenario 1
  test('#1.1 When user hasn’t searched for a city, show upcoming events from all cities.', ({ given, when, then }) => {
  ...
  // Scenario 2
  test('#1.2 User should see a list of suggestions when they search for a city.', ({ given, when, then }) => {
    let AppComponent;
  ...
  // Scenario 3
  test('#1.3 User can select a city from the suggested list.', ({ given, and, when, then }) => {
    let AppComponent;
    let AppDOM;
    let CitySearchDOM;
    let citySearchInput;
